...
func TestParserContextsCarryTheConsumerSeams(t *testing.T) {
	...
	want := []struct {
		...
		{
			expr: "p.Integrations[integrations.GENERIC].RecordOutgoing(parserCtx, genericSession)",
			why: "the generic record branch is the third one, and an unrecognised protocol's egress is " +
				"still egress a consumer worker can make while handling a message",
		},
		{
